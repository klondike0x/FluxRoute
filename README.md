...
> [!IMPORTANT]
> Это обновление направлено на повышение стабильности работы и исправление критических ошибок.

- **⚡ Асинхронное ядро:** Полностью переработана логика управления процессами. Теперь интерфейс никогда не зависает при запуске или остановке стратегий.
- **🛡️ Надежная остановка:** Улучшен механизм завершения winws.exe и других движков. Больше никаких «осиротевших» процессов, мешающих работе.
- **🤖 Исправлен TG WS Proxy:** Исправлена ошибка запуска (ModuleNotFoundError), вызванная отсутствием файлов в портативной версии.
- **💾 Экспорт конфигов:** ИИ-оркестратор теперь автоматически сохраняет все удачные конфигурации Amnezia WG (Warp) в папку `exported_configs`.
