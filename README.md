...
> [!IMPORTANT]
> **Это официальный репозиторий FluxRoute Desktop.**
>
> Форки разрешены условиями GPL-3.0-or-later, но они не являются официальными сборками FluxRoute и могут содержать собственные изменения.

> [!CAUTION]
> ### ⚠️ Осторожно: неофициальные копии
>
> **Единственный официальный источник FluxRoute — [этот репозиторий](https://github.com/klondike0x/FluxRoute).**
>
> Если вы скачали программу из другого места, будьте осторожны. Неофициальные сборки могут содержать:
> - ⚠️ Устаревший код без актуальных исправлений
> - ⚠️ Непроверенные изменения или дополнительную телеметрию
> - ⚠️ Нарушения условий GPL-3.0-or-later, например удаление информации об авторстве
> - ⚠️ Нестабильные или непроверенные версии
>
> **Как защитить себя:**
> - ✅ Всегда проверяйте источник: `github.com/klondike0x/FluxRoute`
> - ✅ Скачивайте файлы только из официальных [релизов](https://github.com/klondike0x/FluxRoute/releases)
> - ✅ Для проверки целостности используйте SHA-256 и GPG-подписи, опубликованные вместе с релизом
> - ✅ Если копия удаляет авторство, выдаёт себя за официальный проект или содержит подозрительные изменения — сообщите сопровождающим проекта
>
> В официальной версии FluxRoute нет намеренной пользовательской телеметрии. Поведение сторонних сборок этим заявлением не покрывается.

**FluxRoute Desktop** — современная GUI-оболочка для управления профилями [`Flowseal/zapret-discord-youtube`](https://github.com/Flowseal/zapret-discord-youtube) с **самообучающимся ИИ-оркестратором** на основе Thompson Sampling и генетической эволюции стратегий.
...
| 🔐 **Immutable Releases**            | Workflow не позволяет повторно опубликовать релиз с уже существующим тегом    |
...
| 👮 **Управление правами** | UAC-запрос при необходимости запуска сетевой защиты + понятные сообщения об ошибках |
...
| 💾 **Диагностический бандл**   | Экспорт ZIP с логами, настройками и системной информацией для отладки; перед отправкой проверьте содержимое |
...
| 📖 **Открытый исходный код** | GPL-3.0-or-later — каждый может проверить, что делает приложение |
...
FluxRoute — GUI с полноценной ИИ-подсистемой, ориентированной на автоматический подбор и эволюцию стратегий:
...
> **Главное отличие:** FluxRoute автоматически подбирает и эволюционирует стратегии под вашу сеть через Thompson Sampling и генетический алгоритм. Таблица отражает заявленные возможности проектов на момент последнего обновления README — перед выбором проверьте их актуальные репозитории.
...
- **Windows 10/11 x64**
- **Права администратора** могут потребоваться для запуска `winws.exe` и WinDivert; само GUI может запускаться без повышения прав

> **Быстрый путь для большинства пользователей:** скачайте последний официальный релиз, установите или распакуйте его в доступную для записи папку и запустите FluxRoute.exe. При необходимости приложение запросит UAC для запуска сетевой защиты. Затем пройдите Onboarding и выберите сервисы для проверки.
...
3. Запустите `FluxRoute.exe` обычным способом. Если потребуется доступ к WinDivert или `winws.exe`, подтвердите запрос UAC.
4. Если `engine/` отсутствует, дождитесь его автоматической загрузки с Flowseal и перезапустите FluxRoute. Для загрузки требуется интернет-соединение.
...
Мод может запускать `.bat`, `.exe`, `.ps1` и `.py` с правами текущего пользователя. Поэтому устанавливайте моды только из доверенных источников и проверяйте их содержимое перед активацией.
...
>
> 1. Проверить логи на вкладке **Логи**
> 2. Обновить `engine/` на вкладке **Обновления**
> 3. Запустить **Сканировать все профили** на вкладке **Оркестратор**
> 4. Запустить расширенную диагностику
>
> Если ошибка связана с запуском `winws.exe` или WinDivert, подтвердите запрос UAC.
...
- При смене провайдера DoH иногда может потребоваться повторное применение настроек или перезапуск FluxRoute. Подробности: [issue #90](https://github.com/klondike0x/FluxRoute/issues/90).
- Для запуска сетевой защиты через `winws.exe` и WinDivert могут потребоваться права администратора. Само GUI при этом может работать без повышения прав.
- Диагностический бандл может содержать настройки, логи и сведения о системе. Перед отправкой разработчикам проверьте архив и удалите чувствительные данные.
- Моды могут запускать `.bat`, `.exe`, `.ps1` и `.py` с правами текущего пользователя. Устанавливайте моды только из доверенных источников.
...
- Сначала проверьте, что FluxRoute скачан из официального репозитория, а хеш и подпись релиза совпадают.
- Не отключайте антивирус полностью и не добавляйте непроверенные сборки в исключения.
- Если антивирус блокирует проверенный официальный файл, добавьте в исключения только папку FluxRoute и только на время диагностики.
- При сомнениях сохраните имя обнаружения и отправьте его вместе с логами сопровождающим проекта.
...
- ✅ **SHA-256 хеши** — в релизе публикуется манифест `SHA256SUMS.txt`
- ✅ **GPG-подписи** — манифест SHA-256 подписывается отдельным ключом релиза
- ✅ **Immutable-релизы** — workflow не позволяет повторно заменить уже опубликованный релиз
...
> [!TIP]
> Официальная сборка FluxRoute получает обновления из репозитория `klondike0x/FluxRoute`; адрес источника задаётся в `AppUpdaterService`. Форкам следует настроить собственный endpoint обновлений, чтобы не отправлять их пользователей на оригинальные сборки.
>
...
- .NET 10 SDK
- Visual Studio 2022 или новее с workload **.NET Desktop Development**, либо JetBrains Rider
...
3. Соблюдайте правила коммитов из [AGENTS/11-GIT-COMMITS.md](AGENTS/11-GIT-COMMITS.md)
...
> [!NOTE]
> ### 📜 Для пользователей и авторов форков
>
> FluxRoute распространяется на условиях **GPL-3.0-or-later**. Лицензия разрешает использовать, изучать, изменять и распространять проект при соблюдении её условий.

<details>
<summary><b>🔍 Раскрыть — основные условия GPL-3.0-or-later</b></summary>
<br/>

Данное ПО распространяется под лицензией **GNU General Public License v3.0 or later**.
...
### Основные обязанности при распространении изменений

Если вы распространяете изменённую версию FluxRoute, необходимо соблюдать условия GPL-3.0-or-later, в частности:

#### 1. Сохранение уведомлений об авторстве и изменениях
Не удаляйте существующие уведомления об авторстве, лицензии и происхождении кода. Для изменённой версии укажите, что она основана на FluxRoute, и опишите существенные изменения.
...
#### 2. Сохранение лицензии и исходного кода
При распространении бинарных файлов предоставьте соответствующий исходный код или корректное письменное предложение о его предоставлении в соответствии с GPL-3.0-or-later. Сохраните текст лицензии и уведомления о лицензировании.

#### 3. Отделяйте форк от официальной версии
GPL разрешает создавать форки. При этом вопросы использования названия, логотипа и оформления относятся отдельно к товарным знакам и не должны вводить пользователей в заблуждение относительно происхождения сборки. Явно укажите, что это форк, если распространяете изменённую версию.

### 💡 Рекомендации (не являются требованиями GPL)

- **Версионирование:** рекомендуется следовать SemVer и ясно отделять версии форка от официальных релизов
- **Приватность:** официальная версия FluxRoute не содержит намеренной пользовательской телеметрии
- **Прозрачность:** не вводите пользователей в заблуждение относительно происхождения кода

### ⚖️ Если условия лицензии нарушены

При нарушении условий GPL права на распространение могут прекратиться в соответствии с §8 GPL-3.0-or-later. Лицензия предусматривает восстановление прав после устранения нарушения в предусмотренных ею случаях.

Вопросы о нарушении авторства, подмене официального проекта или распространении подозрительных бинарных файлов рассматриваются отдельно и требуют конкретных доказательств.
...
FluxRoute был вдохновлён UX- и продуктовыми решениями следующих проектов. Указанные благодарности не означают, что сторонние авторы являются соавторами кода FluxRoute.
...
Проект распространяется по лицензии **GNU General Public License v3.0 or later**.
