...
> **Быстрый путь для большинства пользователей:** скачайте последний релиз, распакуйте его, запустите FluxRoute.exe от имени администратора, обновите engine/, выберите профиль и нажмите «Запустить».
...
Установка TG WS Proxy автоматически переключается на рабочие зеркала PyPI (Tsinghua, Aliyun, USTC), если pypi.org недоступен. В большинстве случаев ручное вмешательство не требуется.

Если автоматический fallback не сработал:

1. Скачайте совместимый Windows embeddable package Python с [официальной страницы загрузок](https://www.python.org/downloads/windows/).
...
Эта ошибка обычно означает, что TG WS Proxy установлен неполностью. Переустановите его на вкладке «TG Прокси». Если проблема сохраняется, удалите папку tg-proxy и повторите установку.
