...
![Auto-Tune Overlay](assets/auto-tune-overlay.png)
*Скриншот оверлея Auto-Tune (изображение будет добавлено в следующем обновлении assets).*
...
FluxRoute поддерживает систему модов — внешних скриптов и конфигов, которые можно включать и выключать через интерфейс. Каждый мод — папка в `mods/` с `manifest.json`.

См. [MODS.md](MODS.md) — полная инструкция по созданию и управлению модами.

Быстрый старт:

1. Создайте папку `mods/my-mod/`
2. Добавьте `manifest.json` и `start.bat`
3. Откройте FluxRoute → вкладка «Моды» → «Включить»
