...
- Создайте проект React с помощью create-vite: `npm init --yes vite@latest fluxroute.v2.frontend -- --template=react-ts  --no-rolldown --no-immediate`.
- Обновление `vite.config.ts` с портом.
- Создание файла проекта (`fluxroute.v2.frontend.esproj`).
